...
import { Github, Linkedin, Mail, Instagram, Sparkles, ArrowRight } from "lucide-react"
import AOS from 'aos'
...
const FloatingParticles = memo(() => {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
      {particles.map((p) => (
        <div key={p.id} className="absolute rounded-full bg-blue-400/20 animate-pulse"
          style={{ width: p.size, height: p.size, left: `${p.left}%`, top: `${p.top}%`, animationDelay: `${p.delay}s`, animationDuration: `${p.duration}s` }}
        />
      ))}
    </div>
  );
    ...
    <button className="group relative w-[140px] sm:w-[160px]">
      ...
      <div className="relative h-10 sm:h-11 bg-[#020d1a] backdrop-blur-xl rounded-lg border border-white/10 leading-none overflow-hidden">
        ...
        <span className="absolute inset-0 flex items-center justify-center gap-2 text-xs sm:text-sm group-hover:gap-3 transition-all duration-300">
          ...
          <Icon className="w-3 h-3 sm:w-4 sm:h-4 text-gray-200 group-hover:translate-x-1 transform transition-all duration-300 z-10" />
        </span>
      </div>
    </button>
  </a>
...
const WORDS = ["Software Engineering Student", "GIS & Data Enthusiast", "Machine Learning Explorer"];
const SOCIAL_LINKS = [
            ...
            <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-12 py-16 sm:py-20 lg:py-24 min-h-screen">
              ...
              <div className="w-full lg:w-1/2 space-y-4 sm:space-y-6 text-center lg:text-left order-1 lg:order-1"
                ...
                <div className="flex flex-row gap-3 justify-center lg:justify-start" data-aos="fade-up" data-aos-delay="1400">
                  <CTAButton href="#ConnectWithMe" text="Connect" icon={ArrowRight} />
              ...
              <div className="w-full lg:w-1/2 flex items-center justify-center order-2 lg:order-2 mt-6 lg:mt-0"
                ...
                <div className="relative group w-full max-w-md sm:max-w-lg lg:max-w-full">
                  {/* Glow border efek */}
                  <div className="absolute -inset-0.5 bg-gradient-to-r from-[#1d4ed8] to-[#0ea5e9] rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-500" />
                  <div className="relative rounded-2xl overflow-hidden">
                    <iframe
                    src="https://www.youtube.com/embed/tBKH0hLIRvY?autoplay=1&mute=1&controls=0&rel=0&modestbranding=1&playsinline=1&iv_load_policy=3&fs=0"
                    className={`w-full h-[200px] sm:h-[280px] md:h-[320px] lg:h-[400px] transition-all duration-500 ${isHovering ? "scale-[98%] lg:scale-[95%] rotate-1" : "scale-100 lg:scale-[98%]"}`}
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
                    title="Muhammad Riskan Rajabi — Travel Video"
                    />
                  </div>
                </div>
              </div>
