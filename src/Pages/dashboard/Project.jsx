...
import { FolderKanban, Upload, Trash2, ImageIcon, Plus } from 'lucide-react'
  ...
  return (
    <div className="relative group">
      ...
      <div className="relative bg-white/5 border border-white/12 rounded-2xl overflow-hidden">
        {!imgLoaded && <div className="w-full aspect-video bg-white/5 animate-pulse" />}
        <img
          src={item.img}
          alt="Project"
          onLoad={() => setImgLoaded(true)}
          className={`w-full aspect-video object-cover group-hover:scale-105 transition-transform duration-500 ${imgLoaded ? 'block' : 'hidden'}`}
        />
        {item.description && (
          <div className="p-3">
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-500 text-xs">{loading ? 'Loading...' : `${items.length} project total`}</p>
        </div>
      </div>
      ...
      <Card>
        <div className="p-5 sm:p-6 space-y-4">
          <h2 className="text-sm font-semibold text-white flex items-center gap-2">
            <Plus className="w-4 h-4 text-blue-400" /> Tambah Project
          </h2>
          <label
          ...
          >
            {preview ? (
              ...
              <div className="text-center space-y-2 p-6">
                ...
                <p className="text-sm text-gray-300">Drag & drop atau klik untuk upload gambar</p>
          ...
          <textarea
            ...
            placeholder="Deskripsi singkat project ini... (opsional)"
      ...
      {loading ? (
        ...
        <Card>
          <div className="p-16 text-center">
            ...
            <p className="text-gray-500 text-sm">Belum ada data project.</p>
          </div>
        </Card>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
          {items.map(item => <ProjectCard key={item.id} item={item} onDelete={deleteItem} />)}
        </div>
      )}
    </div>
  )
